...
| 操作类型   | 默认锁类型             | 什么时候锁“单行”？              | 什么时候锁“多行”？                 | 什么时候看起来像“锁表”？                 | 什么时候是真·锁表？                 |
| ---------- | ---------------------- | ------------------------------- | ---------------------------------- | ---------------------------------------- | ----------------------------------- |
| **INSERT** | 行锁（插入意向锁）     | 只要主键/唯一键不冲突 → 一行    | 批量插入多行                       | 几乎不会（除唯一冲突重试）               | ❌ 不会（除非 LOCK TABLE 或 MyISAM） |
| **UPDATE** | 行锁 + 间隙锁 + 临键锁 | WHERE 命中主键/唯一索引，仅一行 | WHERE 用普通索引匹配多行；范围查询 | WHERE 不走索引 → 全表扫描 → 锁全表所有行 | ALTER/LOCK TABLE 才会真锁表         |
| **DELETE** | 行锁 + 间隙锁 + 临键锁 | WHERE 命中主键/唯一索引，仅一行 | WHERE 用普通索引匹配多行；范围删除 | WHERE 不走索引 → 全表扫描 → 锁全表所有行 | ALTER/LOCK TABLE 才会真锁表         |

示例：

| 情况                                              | 说明             | 示例                                                     |
| ------------------------------------------------- | ---------------- | -------------------------------------------------------- |
| 命中主键 → 锁行                                   | 精确定位一行     | `UPDATE user SET age=20 WHERE id=10;`                    |
| 命中普通索引/范围 → 锁多行                        | 多条记录被锁     | `UPDATE user SET vip=1 WHERE age=20;`                    |
| 没用索引 → 锁全表所有行                           | 全表扫描全部加锁 | `UPDATE user SET status=1 WHERE name='Tom';`             |
| DDL（修改表结构）/LOCK TABLE（手动锁表） → 真锁表 | 完整表锁         | `ALTER TABLE user ADD COLUMN x;``LOCK TABLE user WRITE;` |
...
`select * from employee where age < 20;`那肯定要回表，因为你要的是整行数据（select *）
...
表结构和数据如下所示:
![图片](https://mmbiz.qpic.cn/mmbiz_png/7VkkuTzAZPp08uy3ZghjeKeEiae1ooyibicw8EVmJ8BG8Khy2jYibfM5MwZMYpE5OXFlsQ9JD0rJveYJLTB8hT9Taw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
![图片](https://mmbiz.qpic.cn/mmbiz_png/7VkkuTzAZPp08uy3ZghjeKeEiae1ooyibicKAW3OPoFVWYoqnlzY7Kkwx6Tuib3P5jh88V1IZWMJ3nwjoU3kZx1bCA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)    
测试用例如下:![图片](https://mmbiz.qpic.cn/mmbiz_png/7VkkuTzAZPp08uy3ZghjeKeEiae1ooyibich9nJAgXMicgAwhLiactz1dhbM8knHhptCQdAqNFia0LK19c6mGGpw7ic4w/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
日志分析如下:
...
表结构如下，无数据:  
![图片](https://mmbiz.qpic.cn/mmbiz_png/7VkkuTzAZPp08uy3ZghjeKeEiae1ooyibicmQsgsOgYeUx4AC9hlfcXzRjFITIQ07yd3c1mbnO6f8pPHUSaDKFrKQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
测试用例如下:
![图片](https://mmbiz.qpic.cn/mmbiz_png/7VkkuTzAZPp08uy3ZghjeKeEiae1ooyibicVWZujhIYpkiaFzorMic6jWsVYm2lENaIwAvQKGEIicGibELibRpuTRRcJoA/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
死锁分析:
