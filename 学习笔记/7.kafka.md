...
| -------------------- | ------------------------------------------- | ------------------------------- | ---------------------------------------- | ---------------------------------------------- |
| **所属公司**         | Mozilla                                     | Apache                          | Alibaba                                  | Apache                                         |
| **开源协议**         | MPL                                         | Apache                          | Apache                                   | Apache                                         |
| **开发语言**         | **Erlang（高并发语言）**                    | Java                            | Java                                     | Scala + Java                                   |
| **客户端支持语言**   | Erlang、Java、Ruby、Python、Go 等，支持最广 | Java、C++、Python、PHP、Node 等 | Java 最好，其他语言需社区扩展            | Java、Scala、Python、Go 等                     |
| **协议支持**         | **AMQP、XMPP、SMTP、STOMP（最丰富）**       | OpenWire、STOMP、REST、AMQP     | 自定义协议、JMS-like                     | 自定义协议，社区封装 HTTP                      |
| **消息模型**         | 多种路由、Topic、直接、延迟队列等           | 支持 JMS Queue/Topic            | Topic + Tag、顺序消息、事务消息          | Topic + Partition（分区）                      |
| **Push / Pull**      | **Push 为主，可 Pull**                      | Push+Pull                       | Push/Pull                                | **以 Pull 为主**                               |
| **高可用模式（HA）** | master/slave 镜像队列                       | 基于 ZooKeeper 的 Master-Slave  | **Master / Slave + NameServer**          | **replica 副本集 + leader/follower（最成熟）** |
| **吞吐量**           | 万级（1w-5w/s）                             | 万级（低于 RabbitMQ）           | **十万级（10w+/s）**                     | **百万级（100w+/s）吞吐之王**                  |
| **消息可靠性**       | 可靠性高（ack + 持久化 + 多副本）           | 一般                            | 强一致性、事务消息                       | 强一致性、副本机制（ISR）                      |
| **消息顺序性**       | 支持（单队列）                              | 支持                            | **天然支持严格顺序消息**                 | **分区内有序、跨分区无序**                     |
| **延迟**             | **毫秒级，实时性最强**                      | 中等                            | 中等偏低                                 | 毫秒级~秒级（取决于 flush 策略）               |
| **扩展性**           | 中等                                        | 一般                            | 强（分布式部署）                         | **极强（天然分布式 + 分区）**                  |
| **管理界面**         | 官网管理 UI 完善                            | 开箱即用                        | 控制台一般需部署                         | 第三方工具（Kafka Manager）                    |
| **典型使用场景**     | **订单、库存、延迟消息、复杂路由**          | 企业系统（JMS）、老系统集成     | **电商秒杀、支付、事务消息、大规模并发** | **日志、埋点、行为数据、流式计算**             |
| **优势总结**         | 功能最丰富，延迟低，协议支持最多，易上手    | 实现 JMS、传统系统兼容性好      | **事务消息 + 高吞吐 + 分布式**           | **最高吞吐 + 分布式 + 大数据生态**             |
| **劣势总结**         | 吞吐不如 RocketMQ、Kafka                    | 性能一般，老技术                | 社区生态不如 Kafka 丰富                  | 不适合复杂业务消息（延迟、事务）               |
...
| MQ           | 一句话总结                                            |
| ------------ | ----------------------------------------------------- |
| **RabbitMQ** | 功能最强，延迟最低，跨语言和复杂业务流程的首选        |
| **ActiveMQ** | 老牌 MQ，适合企业级 JMS 场景                          |
| **RocketMQ** | 阿里研发，为分布式高并发和事务消息而生（Java 系最佳） |
| **Kafka**    | 吞吐量之王，是日志、埋点、流式计算的事实标准          |
...
## 11.Kafka 的可靠性是怎么保证的？
###  Kafka 不丢、不乱、不重复的秘密：可靠性 + 幂等性一篇总结（面试必备）

Kafka 为什么能在高并发场景中一直稳如老狗？
核心靠两大能力：
...
Kafka 从 **生产端 → Broker → 消费端** 多链路保证消息不丢。

#### ① acks：控制写入成功的“严格程度”

Kafka 用 `acks` 让你选择写入的可靠等级：

- **acks = 0**：最快但可能丢
- **acks = 1**：Leader 写入成功即可（Leader 崩溃可能丢）
- **acks = -1 / all（最高可靠）**：ISR 所有副本都写成功

👉 **真正的高可靠 = acks = all + 多副本同步**

#### ② min.insync.replicas ≥ 2：高可靠必备条件（关键解释）
...
- 如果 ISR 中只剩 **1 个副本（只有 Leader）**
- 那 “acks = all” 本质 = “acks = 1”
- **Leader 挂了，数据依然会丢**
...
> 要求至少两个副本同步后才允许写入，否则拒绝写入，从根源上避免“单副本写入 → Leader 崩溃 → 数据全丢”。

**真正可靠写入 = acks = all + min.insync.replicas ≥ 2**
...
#### ④ HW（高水位线）+ LEO 保证不读到不一致数据

每个副本维护：

- **LEO**：写到哪了
- **HW（High Watermark）**：所有副本都同步到哪了

消费者只能读取 **HW 之前的数据**
→ 避免 Leader 崩溃导致读到会丢失的数据
→ 保证强一致性

#### ⑤ 手动提交 offset：消费端避免消息丢失
...
###  二、Kafka 的幂等性怎么保证的？（不重复写）

Kafka 幂等性解决的是：

> **Producer 因 retry 导致重复发送同一条消息时，Broker 只写一次**
...
- Producer 启动：得到唯一 PID
- 每个 partition 会为消息生成递增 seq
- Broker 记录每个 `<PID,Partition>` 最新 seq
- 收到重复 seq → 直接丢弃 → 不会写多次
...
- 每次发送都会生成不同的 seq
- Kafka 幂等判断依赖 seq，不看内容

📌 **Kafka 幂等性 ≠ 业务幂等性**
业务层重复发送还是要自己做唯一校验（订单号、Redis SETNX 等）。

### 三、最终 10 秒总结（面试直接说）

> Kafka 可靠性主要依靠 acks、多副本 ISR、HW/LEO 一致性机制以及手动提交 offset 保证消息不丢。真正高可靠写入必须满足 `acks = all` 且 `min.insync.replicas ≥ 2`，确保至少两个副本成功写入后才允许写。
> 幂等性依靠 PID + Partition + SequenceNumber 去重，只避免 retry 写入重复，不负责业务层重复数据的去重。
**1.acks**
