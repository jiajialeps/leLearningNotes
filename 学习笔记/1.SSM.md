...
| **`final` / `static` / `private` 方法 & 构造函数**  | ❌ **不能**                                                 | ✅ **可以**                                                   |
...
graph LR
    %% Java 异常体系的根类
    A["Throwable\n(所有异常和错误的根类)"] --> B["Exception\n(受检异常：必须处理)"]
    A --> C["Error\n(严重错误：JVM级别的问题)"]

    %% Exception 的子类（受检异常）
    B --> B1["IOException\n(IO相关异常，如文件操作失败)"]
    B --> B2["SQLException\n(数据库操作异常)"]
    B --> B3["ReflectiveOperationException\n(反射操作异常)"]
    B --> B4["InterruptedException\n(线程中断异常)"]
    B --> B5["RuntimeException\n(运行时异常：不强制捕获)"]

    %% RuntimeException 的子类（非受检异常）
    B5 --> B5_1["NullPointerException\n(空指针异常)"]
    B5 --> B5_2["ClassCastException\n(类型转换异常)"]
    B5 --> B5_3["ArrayIndexOutOfBoundsException\n(数组越界异常)"]
    B5 --> B5_4["StringIndexOutOfBoundsException\n(字符串越界异常)"]
    B5 --> B5_5["ArithmeticException\n(算术异常，如除0)"]
    B5 --> B5_6["IllegalArgumentException\n(非法参数异常)"]
    B5 --> B5_7["IllegalStateException\n(非法状态异常)"]
    B5 --> B5_8["ConcurrentModificationException\n(并发修改异常)"]
    B5 --> B5_9["NumberFormatException\n(数字格式错误)"]

    %% Error 的子类（严重错误）
    C --> C1["StackOverflowError\n(栈溢出，如递归过深)"]
    ...
    classDef checked fill:#99ccff,stroke:#333,stroke-width:1px;
    classDef runtime fill:#ff9999,stroke:#333,stroke-width:1px;
    classDef error fill:#ff6666,stroke:#333,stroke-width:1px;

    class A root;
    class B,B1,B2,B3,B4 checked;
    class B5,B5_1,B5_2,B5_3,B5_4,B5_5,B5_6,B5_7,B5_8,B5_9 runtime;
    class C,C1,C2,C3,C4 error;
